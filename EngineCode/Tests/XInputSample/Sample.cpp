...
#define INPUT_DEADZONE                                                                                                 \
	(0.24f * FLOAT(0x7FFF)) // Default to 24% of the +/- 32767 range.   This is a reasonable default value but can be
							// altered if needed.
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int)
{
	...
	g_hWnd = CreateWindow(L"XInputSample", L"XInput Sample: SimpleController",
						  ...
						  CW_USEDEFAULT, 600, 600, nullptr, nullptr, hInstance, nullptr);
	...
	for (DWORD i = 0; i < MAX_CONTROLLERS; i++)
	{
		if (g_Controllers[i].bConnected)
		{
			...
			swprintf_s(sz[i], 1024,
					   ...
					   i, (wButtons & XINPUT_GAMEPAD_DPAD_UP) ? L"DPAD_UP " : L"",
					   ...
					   (wButtons & XINPUT_GAMEPAD_A) ? L"A " : L"", (wButtons & XINPUT_GAMEPAD_B) ? L"B " : L"",
					   (wButtons & XINPUT_GAMEPAD_X) ? L"X " : L"", (wButtons & XINPUT_GAMEPAD_Y) ? L"Y " : L"",
					   g_Controllers[i].state.Gamepad.bLeftTrigger, g_Controllers[i].state.Gamepad.bRightTrigger,
					   g_Controllers[i].state.Gamepad.sThumbLX, g_Controllers[i].state.Gamepad.sThumbLY,
					   g_Controllers[i].state.Gamepad.sThumbRX, g_Controllers[i].state.Gamepad.sThumbRY);
		}
		else
		{
