...
void CD3DArcBall::OnBegin(int nX, int nY)
{
	...
	if (nX >= m_Offset.x && nX < m_Offset.x + m_nWidth && nY >= m_Offset.y && nY < m_Offset.y + m_nHeight)
	{
		m_bDrag = true;
		m_qDown = m_qNow;
...
void CBaseCamera::GetInput(bool bGetKeyboardInput, bool bGetMouseInput, bool bGetGamepadInput, bool bResetCursorAfterMove)
{
	...
	//if (bGetKeyboardInput)
	{
		...
		if (Input->KeyHolded(SDL_SCANCODE_LSHIFT))
			MoveSpeed *= 0.5f;
		else if (Input->KeyHolded(SDL_SCANCODE_LCTRL))
			MoveSpeed *= 2.0f;

		// Update acceleration vector based on keyboard state
		if (Input->KeyHolded(SDL_SCANCODE_W))
			m_vKeyboardDirection.z += MoveSpeed;
	...
	if (bGetGamepadInput)
	{
		...
		for (DWORD iUserIndex = 0; iUserIndex < DXUT_MAX_CONTROLLERS; iUserIndex++)
		{
			...
			if (m_GamePad[iUserIndex].wButtons || m_GamePad[iUserIndex].sThumbLX || m_GamePad[iUserIndex].sThumbLX ||
...
void CBaseCamera::UpdateMouseDelta()
{
	...
	if (m_bResetCursorAfterMove)
	{
		...
		SDL_GetWindowSize(MainWindow->GetSDLWindow(), &windowWidth, &windowHeight);

		// Calculate center position
		ptCenter.x = (DisplayMode.w - windowWidth) / 2;
		ptCenter.y = (DisplayMode.h - windowHeight) / 2;
...
{
	m_nActiveButtonMask =
		(bLeft ? MOUSE_LEFT_BUTTON : 0) | (bMiddle ? MOUSE_MIDDLE_BUTTON : 0) | (bRight ? MOUSE_RIGHT_BUTTON : 0);
...
LRESULT CModelViewerCamera::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (uMsg == WM_CAPTURECHANGED)
	{
		if ((HWND)lParam != hWnd)
		{
			if ((m_nRotateModelButtonMask & MOUSE_LEFT_BUTTON) || (m_nRotateModelButtonMask & MOUSE_MIDDLE_BUTTON) ||
				(m_nRotateModelButtonMask & MOUSE_RIGHT_BUTTON))
			{
				m_WorldArcBall.OnEnd();
			}

			if ((m_nRotateCameraButtonMask & MOUSE_LEFT_BUTTON) || (m_nRotateCameraButtonMask & MOUSE_MIDDLE_BUTTON) ||
				(m_nRotateCameraButtonMask & MOUSE_RIGHT_BUTTON))
			{
				m_ViewArcBall.OnEnd();
			}
	...
	if (uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONDBLCLK || uMsg == WM_MBUTTONDOWN || uMsg == WM_MBUTTONDBLCLK ||
		uMsg == WM_RBUTTONDOWN || uMsg == WM_RBUTTONDBLCLK || uMsg == WM_LBUTTONUP || uMsg == WM_MBUTTONUP ||
		uMsg == WM_RBUTTONUP || uMsg == WM_MOUSEWHEEL || uMsg == WM_MOUSEMOVE)
	{
		m_bDragSinceLastUpdate = true;
	}
...
HRESULT CDXUTDirectionWidget::StaticOnD3D9CreateDevice(IDirect3DDevice9* pd3dDevice)
{
	...
	V_RETURN(D3DXCreateEffect(s_pd3d9Device, g_strBuffer, dwBufferSize, NULL, NULL, Flags, NULL, &s_pD3D9Effect, NULL));
...
HRESULT CDXUTDirectionWidget::StaticOnD3D10CreateDevice(ID3D10Device* pd3dDevice)
{
	...
	V_RETURN(pd3dDevice->CreateInputLayout(layout, 2, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize,
										   &s_pVertexLayout));
...
LRESULT CDXUTDirectionWidget::HandleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CAPTURECHANGED: {
		if ((HWND)lParam != hWnd)
		{
			if ((m_nRotateMask & MOUSE_LEFT_BUTTON) || (m_nRotateMask & MOUSE_MIDDLE_BUTTON) ||
				(m_nRotateMask & MOUSE_RIGHT_BUTTON))
			{
				m_ArcBall.OnEnd();
				ReleaseCapture();
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDirectionWidget::OnRender9(D3DXCOLOR color, const D3DXMATRIX* pmView, const D3DXMATRIX* pmProj,
										const D3DXVECTOR3* pEyePt)
{
	m_mView = *pmView;
