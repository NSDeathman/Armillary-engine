...
void CUserInterface::UpdateIngameUI()
{
	...
	{
		Imgui->ShowCursor();
		DummyWindow->ShowCursor();
		HelperWindow->Show();
	}
	else
	{
		Imgui->HideCursor();
		DummyWindow->HideCursor();
		HelperWindow->Hide();
	}

	if (KEY_PRESSED(SDL_SCANCODE_ESCAPE) || GAMEPAD_BUTTON_PRESSED(SDL_CONTROLLER_BUTTON_START) ||
		GAMEPAD_BUTTON_PRESSED(SDL_CONTROLLER_BUTTON_B) || HelperWindow->NeedLeaveToScene())
	{
