...
{
	const float frameTime = Application->GetFrameTime();
	const float frameTimeMS = ConvertToMS(frameTime);
	const float fps = Application->GetFPS();
...
void CMonitoring::Draw()
{
	...
	// Отображение FPS
	ImGui::Text("FPS: %.1f", Application->GetFPS());
	...
	// Время кадра
	const float frameTimeMS = ConvertToMS(Application->GetFrameTime());
