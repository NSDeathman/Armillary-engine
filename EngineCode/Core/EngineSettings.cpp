...
{
	Log("Loading engine settings config file with name: %s", ENGINE_SETTINGS_CONFIG_NAME.c_str());
	...
	if (!ConfigFileExist)
	{
		Log("Can't find engine settings config file, creating new");
		Save();
	}
}
...
{
	Log("Saving engine settings config file");
