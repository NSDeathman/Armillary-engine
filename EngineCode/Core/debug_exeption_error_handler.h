...
namespace Core::Debug
{
    ...
    public:
        ...
        static void logException(const Exception& e, LogLevel level = LogLevel::error);
        static void logStdException(const std::exception& e, LogLevel level = LogLevel::error);
        static void logUnknownException(LogLevel level = LogLevel::error);
        ...
        static bool hasInitialized() { return m_initialized; }
        static void setTerminateOnCritical(bool terminate) { m_terminateOnCritical = terminate; }
    
        // ћетод дл€ обработки ассертов (замена R_ASSERT)
        static void assertHandler(bool condition, 
    ...
    private:
        ...
        static std::string formatAssertionMessage(const std::string& message,
