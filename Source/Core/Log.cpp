...
void CLog::Initialize(const std::string& logDir)
{
	if (m_Initialized)
	{
		PrintInternal("Logger already initialized!");
		return;
	}
	...
	try
	{
		...
		PrintInternal("Logger initialized successfully");
		PrintInternal("Log file: %s", m_LogFilePath.c_str());
	}
...
void CLog::Destroy()
{
	...
	{
		Flush();
		PrintInternal("Destroying logger...");
		m_LogFileStream.close();
		m_Initialized = false;
	}
}
...
void __cdecl CLog::PrintInternal(LPCSTR format, ...)
{
	va_list args;
...
void __cdecl CLog::PrintInternalWithLevel(LPCSTR format, LogLevel level, ...)
{
	va_list args;
	va_start(args, format);
...
void CLog::CreateConsole()
{
	...
	if (AllocConsole())
	{
		...
		PrintInternal("Console created successfully");
	}
	else
	{
