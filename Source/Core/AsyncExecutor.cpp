...
void CAsyncExecutor::Initialize(size_t threadCount)
{
	if (m_Initialized)
	{
		Print("AsyncExecutor already initialized!");
		return;
	}
	...
	if (threadCount == 0)
	{
		...
		if (threadCount == 0)
		{
			...
			Print("Using fallback thread count: %zu", threadCount);
		}
	}
	...
	try
	{
		...
		m_Initialized = true;
		Print("AsyncExecutor initialized with %zu threads\n", threadCount);
	}
	catch (const std::exception& e)
	{
		Print("Failed to initialize AsyncExecutor: %s\n", e.what());
		throw;
	}
}
...
void CAsyncExecutor::Destroy()
{
	if (m_Initialized)
	{
		Print("Destroying AsyncExecutor...");
		Stop();
		m_Initialized = false;
	}
}
...
void CAsyncExecutor::WorkerThread()
{
	while (!stop)
	{
		...
		if (task)
		{
			...
			{
				Print("Exception in async task: %s", e.what());
			}
			catch (...)
			{
				Print("Unknown exception in async task");
			}
			pendingTasks--;
		}
	}
