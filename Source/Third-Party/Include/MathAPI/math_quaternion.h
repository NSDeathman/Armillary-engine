...
class MATH_API quaternion
{
  ...
  public:
	...
	{
		return (std::abs(x) < epsilon) && (std::abs(y) < epsilon) && (std::abs(z) < epsilon) && (std::abs(w) < epsilon);
	}
	...
	static quaternion from_matrix(const float4x4& matrix) noexcept
	{
		...
		const float3x3 rot_matrix(matrix.col0().xyz(), matrix.col1().xyz(), matrix.col2().xyz());
	...
	float3x3 to_matrix3x3() const noexcept
	{
		...
		return float3x3(float3(1 - 2 * (yy + zz), 2 * (xy + wz), 2 * (xz - wy)),
	...
	float4x4 to_matrix4x4() const noexcept
	{
		...
		return float4x4(float4(rot.col0(), 0.0f), float4(rot.col1(), 0.0f), float4(rot.col2(), 0.0f),
 ...
 * @return True if quaternions represent approximately the same rotation
 */
inline bool approximately(const quaternion& a, const quaternion& b,
