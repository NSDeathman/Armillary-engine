...
struct Vertex
{
	...
	Vertex(float x, float y, float z, float u, float v, float nx, float ny, float nz)
		: Position(x, y, z), UV(u, v), Normal(nx, ny, nz), Tangent(0, 0, 0), Bitangent(0, 0, 0)
	{
	}
	Vertex(Math::float3 pos, Math::float3 tangent, Math::float3 bitangent, Math::float3 normal, Math::float2 uv)
		: Position(pos), UV(uv), Normal(normal), Tangent(tangent), Bitangent(bitangent)
	{
	}
};
...
class RENDER_API ShaderPass
{
  public:
	...
	const std::map<std::string, const Texture3D*>& GetTextures3D() const
	{
		return m_textures3D;
	}
	const std::map<std::string, const TextureCube*>& GetTexturesCube() const
	{
		return m_texturesCube;
	}
	const std::map<std::string, const Sampler*>& GetSamplers() const
	{
		return m_samplers;
	}
...
struct RENDER_API RenderMaterial
{
	std::string Name;

	// --- PBR Parameters (Metallic-Roughness Workflow) ---

	// Base Color (Albedo) + Alpha
	Math::float4 BaseColor = {1.0f, 1.0f, 1.0f, 1.0f};

	// PBR Factors
	float Metallic = 0.0f;	// 0.0 = Dielectric (Plastic/Wood), 1.0 = Metal
	float Roughness = 0.5f; // 0.0 = Smooth (Mirror), 1.0 = Rough (Matte)

	// Emissive (Self-illumination)
	Math::float3 Emissive = {0.0f, 0.0f, 0.0f};

	// --- Texture Maps Paths ---

	std::string AlbedoMap;	  // map_Kd
	std::string MetallicMap;  // map_Pm or map_Ks (fallback)
	std::string RoughnessMap; // map_Pr or map_Ns (fallback)
	std::string NormalMap;	  // norm or map_Bump
	std::string EmissiveMap;  // map_Ke
	std::string OcclusionMap; // map_Ka (Ambient Occlusion)
...
struct SubMesh
{
	...
	int MaterialIndex = -1; // Индекс материала в исходном файле (mtl)
...
class RENDER_API Mesh
{
  public:
	...
	bool LoadFromOBJ(const std::string& filepath, const std::string& mtlBaseDir,
	...
	static void GenerateHemisphere(Mesh& outMesh, float radius = 1.0f, int segments = 32, int rings = 16,
	...
	Math::float3 GetBoundsMin() const
	{
		return m_MinBound;
	}
	Math::float3 GetBoundsMax() const
	{
		return m_MaxBound;
	}
	Math::float3 GetBoundsCenter() const
	{
		return (m_MinBound + m_MaxBound) * 0.5f;
	}
	Math::float3 GetBoundsSize() const
	{
  ...
  private:
	...
	Math::float3 m_MinBound = Math::float3(FLT_MAX);
	Math::float3 m_MaxBound = Math::float3(-FLT_MAX);
};
